...
void Assert( bool input, unsigned int line )
{
	if ( !input )
	{
		json::Debug() << "Failed test on line " << line << "\n";
...
{
	float x, y;
	bool operator == ( const PODstruct &pod ) const
	{
...
int main( int, char *[] )
{
	...
	try
	{
		...
		// base64
		expected = json::base64::encode( "ABCD", 4 );
		Test( "QUJDRA==", expected, __LINE__, RoundTrip );

		json::base64::data_type data = json::base64::decode( "QUJDRA==" );
		expected = std::string( data.begin(), data.end() );
		Test( "ABCD", expected, __LINE__, RoundTrip );

		const PODstruct in = { 1.234e13, 11 };
		const PODstruct out = json::base64::decode< PODstruct >( json::base64::encode( in ) );
		Assert( in == out, __LINE__ );
	}
